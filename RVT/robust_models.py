...
class Transformer(nn.Module):
    ...
    def forward(self, x, mask_matrix=None, mask_layer_index=None, is_feat=False):
        B,C,H,W = x.shape
        x = rearrange(x, 'b c h w -> b (h w) c')
        # x = x.permute(0,2,3,1).reshape(B, H * W, C)

        feat = []
        feat_layer_idx = [len(self.blocks) // 4 - 1, len(self.blocks) // 4 * 2 - 1, len(self.blocks) // 4 * 3 - 1, len(self.blocks) - 2, len(self.blocks) - 1]
        ...
        for i in range(self.depth):
            ...
            if is_feat and idx in feat_layer_idx:
                feat_x = rearrange(x, 'b (h w) c -> b c h w', h=H, w=W)
                feat.append(feat_x.clone())
            idx += 1
...
class PoolingTransformer(nn.Module):
    ...
    def forward_features(self, x, is_feat=False):
        x = self.patch_embed(x)
        x = self.pos_drop(x)
        feat = None
        for stage in range(len(self.pools)):
            if is_feat:
                x, feat = self.transformers[stage](x, is_feat=is_feat)
            else:
                x = self.transformers[stage](x)
            x = self.pools[stage](x)

        if feat is None and is_feat:
            x, feat = self.transformers[-1](x, is_feat=is_feat)
        else:
            x = self.transformers[-1](x)
        cls_features = self.norm(torch.flatten(self.gap(x), 1))

        if is_feat:
            return cls_features, feat
        else:
            return cls_features
    ...
    def forward(self, x, x1=None, is_feat=False):
        if x1 is not None:
            x = torch.cat((x, x1), 0)
            if is_feat:
                cls_features, feats = self.forward_features(x, is_feat=is_feat)
            else:
                cls_features = self.forward_features(x)
            output = self.head(cls_features)
            if is_feat:
                return feats, output[0:output.size(0)//2], output[output.size(0)//2:]
            else:
