...
from timm.utils import NativeScaler, get_state_dict, ModelEmaV2, dispatch_clip_grad

from gpd_utils import extract_tiny_net


def get_args_parser():
    parser = argparse.ArgumentParser('ADViT training and evaluation script', add_help=False)
    ...
    parser.add_argument('--deepaug_freq', type=int, default=1, help='deepaug_freq')
    ...
    parser.add_argument('--extra-weight', type=float, default=0, help='extra_weight')
    parser.add_argument('--extra-loss-type', default='standard', type=str, help='extra-loss-type')
    parser.add_argument('--eval_model_ema', action='store_true', help='eval_model_ema')
    parser.add_argument('--epsilon', default=1, type=int)
    parser.add_argument('--pgd-alpha', default=0.5, type=float)
    parser.add_argument('--aux-weight', type=float, default=0, help='aux_weight')

    # extra parameters
    parser.add_argument('--noise-prob', default=0.1, type=float, help="noise_prob")
    parser.add_argument('--num-noisy-tokens', default=0, type=int, help="number of noisy tokens")
    parser.add_argument('--noise-level', default=1.0, type=float, help="noise_level")
    parser.add_argument('--noise-which-layer', default='random', type=str, help='noise_which_layer')
    parser.add_argument('--cifarc_base_path', default='/BS/yong_project1/work/dataset/CIFAR-10-C', type=str, help='imagenet-sketch')
    parser.add_argument('--temperature', default=1.0, type=float, help="temperature")
    parser.add_argument('--tokenaug-type', default='gaussian_noise', type=str, help='tokenaug_type')
    parser.add_argument('--aux-loss-type', default='ce', type=str, help='aux_loss_type')
    parser.add_argument('--num-scales', default=1, type=int, help="number of scale levels")
    ...
    # params for explore transformer
    parser.add_argument('--threshold', default=2, type=int, help="threshold for attention score")
    parser.add_argument('--layer-loss-weight', type=float, default=0, help='layer_loss_weight')
    parser.add_argument('--token-loss-weight', type=float, default=0, help='token_loss_weight')

    # for GPD
    parser.add_argument('--tiny_net_version', dest='tiny_model', default='UNET_v7_3_prelu', help='net_version')
    parser.add_argument('--load_tiny_model_file_path', dest='tiny_ckpt_load', type=str, default=None,
                        help='tiny models are loaded from here')
    parser.add_argument('--tiny_eval_model_ema', action='store_true', help='tiny_eval_model_ema')
    parser.add_argument('--gpd_ratio', type=int, default=1, help='expand for squeeze ratio')
    ...
    parser.add_argument('--avoid_bad_teacher', action='store_true', help='avoid_bad_teacher')
    parser.add_argument('--split_grad', action='store_true', help='split_grad')
    parser.add_argument("--start-gpd-epoch", default=0, type=int, help="start_gpd_epoch")
    ...
    parser.add_argument('--dkd_alpha', type=float, default=1.0, help='dkd_alpha')
...
class MyNativeScaler(NativeScaler):
    state_dict_key = "amp_scaler"

    def __init__(self):
        super().__init__()

    def __call__(self, loss, optimizer, clip_grad=None, clip_mode='norm', parameters=None, create_graph=False, split_grad=False, grad_mask_groups=None, grad_expanded=None):
...
def main(args):
    ...
    if args.teacher is not None:
        logger.info(f"Creating teacher model: {args.teacher}")
        ...
        if args.teacher_weight is not None:
            logger.info(f'Load pretrained teacher from {args.teacher_weight}')
            load_checkpoint(teacher, args.teacher_weight, use_ema=False)
        teacher.to(device)
    else:
        teacher = None
    ...
    if args.use_orepa:
        model = transfer2orepa(model, train_from_scratch=args.pretrain_path is None)

    test_input = torch.rand(1, 3, 224, 224)
    if not args.skip_es:
        model = gpd_init(model, first_module_name_list=['patch_embed.proj.0'], last_module_name_list=['head'], test_input=[test_input], gpd_ratio=args.gpd_ratio)
    else:
        args.gpd_ratio = 1

    if args.use_reviewkd:
        num_classes = model.num_classes
        model = build_review_kd(args.model, model, expand_ratio=args.gpd_ratio if not args.skip_es else 1)
    ...
    if args.resume_path:
        _ = resume_checkpoint(
            model, args.resume_path,
            optimizer=optimizer,
            loss_scaler=loss_scaler,
            log_info=args.local_rank == 0)
        load_checkpoint(model_ema.module, args.resume_path, use_ema=False)
        logger.info(f'Resume checkpoint from {args.start_epoch}-th epoch via {args.resume_path}')
    ...
    if args.eval and args.ckpt_load is not None:
        load_checkpoint(model, args.ckpt_load, use_ema=False)
        if args.deploy:
            extract_model = extract_tiny_net(model, test_sample=torch.rand(1, 3, 224, 224).to(device))
            # convert orepa to conv
            extract_model = replace_orepa_with_conv(extract_model)

            # Save the deploy model
            torch.save(extract_model.state_dict(), args.deploy_model_path)
            logger.info(f"The deploy model is saved at {args.deploy_model_path}")

            # test deploy model
            logger.info("Loading saved deploy model for testing")
            object_model.load_state_dict(torch.load(args.deploy_model_path, map_location=device), strict=True)
            logger.info("Starting evaluation...")
            object_model.to(device)
            test_stats = evaluate(logger, data_loader_val, object_model, device, args=args)
        else:
            logger.info("Starting evaluation...")
            test_stats = evaluate_gpd(logger, data_loader_val, model, device, args=args)
    ...
    if args.eval or args.eval_first:
        ...
        if args.inc_path:
            if args.data_set == 'CIFAR10' or args.data_set == 'CIFAR100':
                corruption_accuracy = eval_cifarc(logger, model, device, args=args)
            elif args.data_set == 'IMNET':
                corruption_accuracy = eval_inc(logger, model, device, args=args)
        ...
        if args.ina_path:
            ...
            test_stats = evaluate_gpd(logger, ina_data_loader, model, device, args=args, indices_in_1k=indices_in_1k)
        ...
        if args.inr_path:
            ...
            test_stats = evaluate_gpd(logger, inr_data_loader, model, device, args=args, indices_in_1k=indices_in_1k)
        ...
        if args.insk_path:
            ...
            test_stats = evaluate_gpd(logger, insk_data_loader, model, device, args=args)
            logger.info(f"Accuracy on the ImageNet-Sketch: {test_stats['acc1']:.1f}%")

        if args.fgsm_test:
            test_stats = evaluate_gpd(logger, data_loader_val, model, device, adv='FGSM')
            logger.info(f"Accuracy of the FGSM: {test_stats['acc1']:.1f}%")

        if args.pgd_test:
            # test_stats = evaluate_clean_attention_noisy_feature(logger, data_loader_val, model, device, adv='PGD', args=args)
            test_stats = evaluate_gpd(logger, data_loader_val, model, device, adv='PGD', args=args)
    ...
    # model.eval()
    # optimizer.zero_grad()
    # test_input = torch.rand(10, 3, 224, 224).to(device)
    # target = torch.rand(10, 1000).to(device)
    # pred = forward_tiny(model, test_input)
    # if args.use_reviewkd:
    #     _, pred = pred
    # loss = criterion(pred, target)
    # loss.backward()
    # grad_mask_groups = build_grad_mask(optimizer)
    # optimizer.zero_grad()
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch_gpd(
            logger, args, model, criterion, data_loader_train,
            optimizer, device, epoch, loss_scaler,
            args.clip_grad, model_ema, mixup_fn,
            set_training_mode=args.finetune == '',  # keep in eval mode during finetuning,
        ...
        if model_ema is not None and not args.model_ema_force_cpu:
            ema_eval_metrics = evaluate_gpd(logger, data_loader_val, model_ema.module, device, args=args)
            test_stats['ema_acc'] = ema_eval_metrics['acc1']
            # if args.num_scales>1 and test_stats['ema_acc'] > test_stats['acc1']:
            #     logger.info("Load parameters from ema")
