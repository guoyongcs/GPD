...
class DynamicOREPA(DynamicModule, OREPA):
    ...
    def export(self) -> nn.Module:  # important
        module = OREPA(
            self.active_in_channels, self.active_out_channels, self.kernel_size, self.stride, self.padding, self.dilation, self.active_groups, self.internal_channels_1x1_3x3, self.deploy, self.nonlinear, self.single_init, self.weight_only, self.single_branch_preserve, self.init_hyper_para, self.init_hyper_gamma
        )
        module.load_state_dict(self.active_state_dict())
        return module
