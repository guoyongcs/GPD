...
class ClassAttentionBlock(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        # TODO mlp
        # preprocess gamma1 and gamma2
        if hasattr(self.attn, 'active_ratio'):
            active_ratio = self.attn.active_ratio
            # gamma1
            new_shape = self.gamma1.shape[0]
            new_shape = new_shape // active_ratio
            ...
            active_gamma1 = self.gamma1
            active_gamma2 = self.gamma2
        ...
        x = x + self.drop_path(active_gamma1 * x_attn)
        if self.tokens_norm:
            x = self.norm2(x)
        else:
        ...
        x_res = x
        cls_token = x[:, 0:1]
        cls_token = active_gamma2 * self.mlp(cls_token)
...
class ChannelProcessing(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., linear=False, drop_path=0., 
        ...
        self.drop = drop
        self.norm_layer = norm_layer
        self.cha_sr_ratio = cha_sr_ratio
...
class FAN(nn.Module):
    ...
    def forward_features(self, x):
        ...
        feat_layer_idx = [len(self.blocks) // 4 - 1, len(self.blocks) // 4 * 2 - 1, len(self.blocks) // 4 * 3 - 1, len(self.blocks) - 1]
        idx = 0
        ...
        try:
            active_ratio = self.patch_embed.backbone.stem[0].active_ratio
            gpd_ratio = self.patch_embed.backbone.stem[0].gpd_ratio
            new_cls_tokens = cls_tokens[:, :, :x.shape[-1]] 
            new_cls_tokens = avg_param_for_tiny_model1d(new_cls_tokens, cls_tokens, x.shape[-1])
            cls_tokens = new_cls_tokens * active_ratio / gpd_ratio
        except:
            cls_tokens = cls_tokens[:, :, :x.shape[-1]]
        ...
        for blk in self.cls_attn_blocks:
            ...
            #     x = blk(x)
        H = int(x.shape[1] ** 0.5)
        last_feat = rearrange(x[:,:H*H], 'b (h w) c -> b c h w', h=H, w=H)
        feat.append(last_feat.clone())
        x = self.norm(x)[:, 0]
        return x, feat

    def forward(self, x, x1=None, is_feat=False):
        if x1 is not None:
            x = torch.cat((x, x1), 0)
            x, feat = self.forward_features(x)
            x = self.head(x)
            if is_feat:
                return feat, x[0:x.size(0)//2], x[x.size(0)//2:]
            else:
...
class PatchOcclusion(nn.Module):
    def __init__(self, mask_ratio, in_chans, patch_size, num_patches):
        super(PatchOcclusion, self).__init__()
        self.mask_ratio = mask_ratio%1
        self.patch_size = patch_size
        self.patch_embed = nn.Conv2d(in_chans, 1, kernel_size=patch_size, stride=patch_size, bias=False)
        self.relu = nn.GELU()
        self.fc = nn.Linear(num_patches, num_patches, bias=False)
        self.ste = StraightThroughEstimator(mask_ratio%1)
    ...
    def forward(self, x):
        ...
        x = self.relu(self.patch_embed(x))
        x = x.reshape(B, -1)
        x = self.fc(x)
        out = self.ste(x)
        # patch fold
        _, N = x.shape
        out = out.reshape(-1)
        out = out[:,None,None,None].expand(B*N, 1, self.patch_size, self.patch_size)
        patches = out.reshape(B, -1, 1, self.patch_size,self.patch_size).permute(0,2,3,4,1).contiguous().reshape(B, self.patch_size*self.patch_size, -1)
        out = F.fold(patches, (H,W), self.patch_size, stride=self.patch_size)
...
class StraightThroughEstimator(nn.Module):
    ...
    def forward(self, x):
        x = STEFunction.apply(x, self.mask_ratio)
        return x
...
class STEFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, r):
        # eps = 1e-4
        # input += eps * torch.rand(input.shape, dtype=input.dtype, device=input.device)
        quantile = torch.quantile(input.to(torch.float), r, dim=1).unsqueeze(-1)
        return (input > quantile).float()
