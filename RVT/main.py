...
import robust_models_mde
import robust_models_llms
import robust_models_mdh
import robust_models_exmd
...
def main(args):
    ...
    if args.eval or args.eval_first:
        ...
        if args.pgd_test:
            ...
            test_stats = evaluate(logger, data_loader_val, model, device, adv='PGD', args=args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # if args.deepaugment and epoch%args.deepaug_freq==0 and epoch<args.remove_deepaug:
        #     dataset_train, _ = build_dataset(is_train=True, args=args)
        # else:
        #     args.deepaugment = False
        #     dataset_train, _ = build_dataset(is_train=True, args=args)
        #     args.deepaugment = True
        ...
        if args.data_set == 'CIFAR10' or args.data_set == 'CIFAR100':
            corruption_accuracy = eval_cifarc(logger, model, device, args=args, target_type='motion_blur')
        elif args.data_set == 'IMNET':
            try:
                corruption_accuracy = eval_inc(logger, model, device, args=args, target_type='motion_blur')
            except:
                pass
        ...
        if epoch==args.epochs-1:
            if args.data_set == 'CIFAR10' or args.data_set == 'CIFAR100':
                ...
                corruption_accuracy = eval_inc(logger, model, device, args=args)
            logger.info(f"mCE: {corruption_accuracy:.1f}%")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser('ADViT training and evaluation script', parents=[get_args_parser()])
