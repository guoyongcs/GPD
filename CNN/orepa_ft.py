...
def transfer2orepa_single(model, attr_name, query_class, single_branch_preserve, train_from_scratch, num_branches_orepa=6):
    ...
    if len(attr_name_split) == 1:
        ...
        if isinstance(source_model, query_class):
            ...
            value = OREPA(source_model.in_channels, source_model.out_channels, source_model.kernel_size[0], source_model.stride[0], source_model.padding[0], source_model.dilation[0], source_model.groups, single_init=not train_from_scratch, single_branch_preserve=single_branch_preserve, num_branches_orepa=num_branches_orepa)
            with torch.no_grad():
...
def transfer2orepa(model, single_branch_preserve=False, return_transfered_list=False, train_from_scratch=False, exclude_module_name_list=[], print_info=True, contain_orepa=False, num_branches_orepa=6):
    ...
    for name_module1 in model.named_modules():
        ...
        if isinstance(m1, query_class) and m1.stride[0]==1 and m1.kernel_size[0]==3 and m1.padding[0] == m1.kernel_size[0] // 2 and name1 not in exclude_module_name_list:
            if print_info:
                print(f'Transfer {name1}: {m1} to OREPA block')
            transfer2orepa_single(new_model, name1, query_class, single_branch_preserve, train_from_scratch, num_branches_orepa=num_branches_orepa)
...
class OREPA(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, internal_channels_1x1_3x3=None, deploy=False, nonlinear=None, single_init=False, weight_only=False, single_branch_preserve=False, init_hyper_para=1.0, init_hyper_gamma=1.0, num_branches_orepa=6):
        ...
        self.num_branches_orepa = num_branches_orepa
